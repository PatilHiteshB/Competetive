...
class Solution:
    ...
    def solve(self, root, target):
        ...
        cleft, left = self.solve(root.left, target)
        cright, right = self.solve(root.right, target)
        
        su = root.data + left + right
        if su == target:
            
            if self.count > 1+cleft+cright:
                self.count = 1+cleft+cright
                self.ans = root
            
        return (1+cleft+cright, su)
    ...
    def minSubtreeSumBST(self, target, root):
        ...
        if self.ans != None:
            if self.isBST(self.ans, -float('inf'), float('inf')):
                return self.count
        return -1
