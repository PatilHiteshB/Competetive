...
class Solution {
  public: int Fact[100001];
  int binExp(int n, int x) {
    int ans = 1;
    while (x) {
      if (x & 1) ans = (ans * 1 LL * n) % mod;
      n = (n * 1 LL * n) % mod;
      x >>= 1;
    }
    return ans;
  ...
  void CreateFactorial() {
    ...
    for (int i = 1; i <= 100000; i++) {
      Fact[i] = (i * 1 LL * Fact[i - 1]) % mod;
    }
  }
  int nCr(int N, int i) {
    // N!/i!*(N-i)!
    int numerator = Fact[N];
    int denominator = (Fact[i] * 1 LL * Fact[N - i]) % mod;
    return (numerator * 1 LL * binExp(denominator, mod - 2)) % mod;
  }
